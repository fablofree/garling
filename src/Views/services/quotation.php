...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .inv-title-center h1 { font-size: 26px; font-weight: 800; color: #7c3aed; letter-spacing: 2px; }
        .inv-title-center p { font-size: 11px; color: #888; margin-top: 4px; }
        ...
        .line-col h4 { font-size: 11px; text-transform: uppercase; color: #475569; font-weight: 700; letter-spacing: 0.5px; margin-bottom: 8px; padding-bottom: 6px; border-bottom: 2px solid #7c3aed; }

        table { width: 100%; border-collapse: collapse; }
        th { background: #f1f5f9; color: #475569; padding: 8px 10px; text-align: left; font-size: 11px; text-transform: uppercase; }
        ...
        .total-line.grand { background: #7c3aed; color: #fff; font-weight: 700; font-size: 15px; padding: 10px 14px; }
        .total-line.grand span { color: #fff; }

        .remarks-section { background: #faf5ff; border: 1px solid #ddd6fe; border-radius: 6px; padding: 14px 16px; margin-bottom: 24px; }
...
<div class="no-print">
    ...
    <button onclick="window.print()" class="btn-print">Print Quotation</button>
</div>
...
<div class="invoice-wrapper">
    ...
    <div class="inv-header">
        ...
        <div class="inv-meta-right">
            <?php if (!empty($quotation_number)): ?>
            <div style="font-size:16px;font-weight:700;color:#1e293b">No: <?= htmlspecialchars($quotation_number, ENT_QUOTES, 'UTF-8') ?></div>
            <?php endif; ?>
            <p>Date: <?= htmlspecialchars($e['entry_date'] ?? '', ENT_QUOTES, 'UTF-8') ?></p>
            <?php if (!empty($e['delivery_date'])): ?>
            <p>Valid until: <?= htmlspecialchars($e['delivery_date'], ENT_QUOTES, 'UTF-8') ?></p>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="totals-box">
        <div class="totals-inner">
            ...
            <?php endif; ?>
            <div class="total-line grand"><span>TOTAL ESTIMATE</span><span><?= $fmt((float)($e['total_cost'] ?? 0)) ?></span></div>
        </div>
    </div>
